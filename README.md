...
- **Nothing here is production-hardened.** Several subsystems described in detail further down are working prototypes rather than finished, battle-tested components. In particular:
  ...
  - **Multi-Agent Debate** (`reasoning/debate/`) is **role-conditioned prompting of a single model, not routing between distinct MoE experts.** Concretely: `_get_initial_thought()` and `_expert_review()` in `council.py` select a per-`exp_id` system prompt from a static `ROLE_TEMPLATES` table, then call `bridge.generate(ids, max_new_tokens=...)` — the same two-argument call, unchanged, regardless of which "expert" is speaking. There is no router mask, no expert-specific routing bias, and no forced expert selection anywhere in this call chain; the docstring's own description — "all experts from a single Qwen 3.5 9B Instruct source" — confirms it's one model invoked repeatedly with different system-prompt text, not distinct experts being routed to. "Council Debate (8 experts)" means 8 role-conditioned passes over one model, not 8 separate models or genuinely separate MoE experts. When no model is connected at all, the mechanism falls back further still, to predefined text templates and keyword overlap — a stub for exercising the pipeline, not a claim about debate quality.
    This design choice is deliberate, not a stand-in for a missing feature: the goal of the debate step is reducing hallucination, not maximizing raw capability (which depends on parameter count, training data, and compute — things the debate step doesn't touch). A model hallucinates largely because it commits early to one chain-of-thought and never doubts it; cycling the same model through several role-conditioned passes that see each other's answers forces exactly that cross-checking, cheaply. A true router-based debate between separately-trained MoE experts would give genuinely distinct perspectives rather than prompt-conditioned ones, but at the cost of training multiple experts instead of one — the harder and more expensive version of the same idea. Role-conditioned prompting is the cheap version that still captures the main benefit (forced skepticism across passes) without that training cost, which is why it's what's actually implemented today.
  ...
  - The **default `ModelConfig`** in `modeling/config.py` is a small experimental configuration (e.g. `d_model=128, n_layers=4, d_ff=512, n_experts=8`), not a 9-billion-parameter model. Weights are randomly initialized on construction — there is no trained checkpoint of any size shipped with this repo, and no documented training run has produced one yet. Every quantization, KV-cache, and speculative-decoding claim in this README describes the *runtime's behavior on whatever weights you load into it* (random or externally trained), not the quality of a Draco-trained model, because no such model currently exists. Running `DracoTransformerV1()` with defaults gives you an untrained ~small model for exercising the code path, not a usable language model.
  - The **`reasoning` verifier** in the hallucination subsystem (§1.3) assigns moderate trust (0.55) to a claim if it's traceable from the model's own selected reasoning trace. If that reasoning trace was generated by the same model being evaluated, this is evidence *of self-consistency*, not independent *evidence of correctness* — a confidently wrong chain of reasoning can still register as "traceable" and nudge the fused score toward "supported." Weigh `reasoning` and `planner` (0.4) as the softest signals in the fusion step; `tool` (0.85), `numerical` (0.9), and `citation` (0.9) are the ones checked against something external to the generating model.
